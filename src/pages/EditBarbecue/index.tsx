...
const registerFormSchema = zod.object({
  ...
  barbecueDate: zod.string().refine( (value) => {
  ...
  numberOfMen: zod.string().refine( (value) =>
        value.trim() !== "" && !isNaN(Number(value)) && Number(value) >= 0,
        { message: "Digite um valor válido.", }
    ),

  numberOfWomen: zod.string().refine( (value) =>
        value.trim() !== "" && !isNaN(Number(value)) && Number(value) >= 0,
        { message: "Digite um valor válido.", }
    ),

  numberOfchildren: zod.string().refine( (value) =>
        value.trim() !== "" && !isNaN(Number(value)) && Number(value) >= 0,
    ...
    async function fetchData() {
      try {
        if (id) {
          ...
          (barbecueData) ? setFormValues(barbecueData)
          : console.log("Dados do churrasco não encontrados na resposta da API.");
        }
      } catch (error) { console.log("Erro ao obter dados do churrasco:", error); }
    }
    fetchData();
  }, [id, setValue]);
    ...
    <main>
      { 
        ...
        ? <h1 className="flex items-center gap-2 text-xl sm:text-2xl font-semibold border-b-2 border-amber-500 pb-4 mb-8">
          <PenSquare size={24} /> Editar Churrasco</h1>
        : <h1 className="flex items-center gap-2 text-xl sm:text-2xl font-semibold border-b-2 border-amber-500 pb-4 mb-8">
